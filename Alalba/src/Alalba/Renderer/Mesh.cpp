...
namespace Alalba {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			ALALBA_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		ALALBA_CORE_INFO("Loading mesh: {0}", filename.c_str());

		Assimp::Importer importer;

		const aiScene* scene = importer.ReadFile(filename, ImportFlags);
		if (!scene || !scene->HasMeshes())
			ALALBA_CORE_ERROR("Failed to load mesh file: {0}", filename);

		aiMesh* mesh = scene->mMeshes[0];

		ALALBA_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
		ALALBA_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
		...
		{
			ALALBA_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
