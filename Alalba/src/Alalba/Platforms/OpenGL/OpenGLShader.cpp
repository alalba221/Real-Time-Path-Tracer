...
#if UNIFORM_LOGGING
#define ALALBA_LOG_UNIFORM(...) ALALBA_CORE_WARN(__VA_ARGS__)
#else
#define ALALBA_LOG_UNIFORM
#endif
	...
	void OpenGLShader::ReadShaderFromFile(const std::string& filepath)
	{
		...
		{
			ALALBA_CORE_WARN("Could not read shader file {0}", filepath);
		}
	}
	...
	void OpenGLShader::CompileAndUploadShader()
	{
		...
		while (pos != std::string::npos)
		{
			...
			ALALBA_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1;
			std::string type = m_ShaderSource.substr(begin, eol - begin);
			ALALBA_CORE_ASSERT(type == "vertex" || type == "fragment", "Invalid shader type specified");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				ALALBA_CORE_ERROR("Shader compilation failed:\n{0}", &infoLog[0]);

				// We don't need the shader anymore.
				glDeleteShader(shaderRendererID);

				ALALBA_CORE_ASSERT(false, "Failed");
			}
