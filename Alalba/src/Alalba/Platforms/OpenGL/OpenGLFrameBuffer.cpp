...
namespace Alalba {
	...
	void OpenGLFrameBuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		ALALBA_RENDER_S({
			...
			// TODO: Create Alalba texture object based on format here
			...
			else if (self->m_Format == FrameBufferFormat::RGBA32F)
			{
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, self->m_Width, self->m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
			}
			...
			if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
				ALALBA_CORE_ERROR("FrameBuffer is incomplete!");
