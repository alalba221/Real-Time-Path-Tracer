...
namespace Alalba {
	...
	void OpenGLTexture2D::ReloadFromMemory(unsigned char* data,unsigned int width, unsigned int height)
	{
		auto self = this;
		m_Width = width;
		m_Height = height;
		...
		ALALBA_RENDER_2(self, data, {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, self->m_Width, self->m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, (void*)data);
			glBindTexture(GL_TEXTURE_2D, 0);
